...
-   HDFS权限配置
    ...
    HDFS有权限设置，确定老集群是否有acl规则，是否要同步，检查dfs.permissions.enabled 和dfs.namenode.acls.enabled的配置新旧集群是否一致，按照实际需要修改。
...
-   操作步骤：
    ...
    8.  meta数据全部订正后，就可以重启metaserver了。命令行hive，查询库和表，查询数据，验证正确性。
...
-   Flume分区表写入
    ...
    Flume数据双写，双写时需控制开始的时机，要保证flume在开始一个新的时间分区的时候来进行新集群的同步。 如flume每小时整点会同步所有的表，那就要整点来之前，开启flume同步服务，这样flume在一个新的小时内写入的数据，在老集群和新集群上是完全一致的。而不完整的老的数据会在distcp的时候全量的同步会覆盖它。而开启双写时间点后的新数据，在数据同步的时候不进行同步。 这个新的写入的数据，我们在划分数据阶段，记得不要放到数据同步的目录里。


## 作业同步 {#section_zrh_jly_wfb .section}

Hadoop，Hive，Spark，MR等如果设计较大的版本升级，可能设计作业改造，要视具体情况而定。
