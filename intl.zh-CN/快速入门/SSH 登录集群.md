...
1.  使用如下命令 SSH 登录到 master 主机。请在[ZH-CN\_TP\_17857\_V1.md\#](intl.zh-CN/用户指南/集群配置/集群详情（不发布）.md#)的主机信息栏中获取集群 master 机器的公网 IP。
...
-   方式一：端口动态转发
    ...
    -   用户名密码的方式，需要输入密码：
        ...
        **说明：** 8157 是您本地机器没有被使用过的任何一个端口，用户可以自定定义。
